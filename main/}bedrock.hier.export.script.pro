...
#Region @DOC
# Description:
# This process exports a dimension hierarchies as _TurboIntegrator script_. Thas is rather than creating a CSV which can be read by a generic 
...
# as 100% stand-alone without need for data source o any parameters for the process. This can be useful in sutuations where DevOps has no file system access
...
# * pDimInfo 1/0 numeric Boolean whether to include metadata relevant to the dimensions/hierarches defined in pDim from }DimensionProperties, }HierarchyProperties and }DimensionAttributes
...
# * There is no Boolean for exporting dimension structure, this is determined by pEle. To export all elements use pEle="*". To supress export of rollup structure (and attribute values) use pEle=""
...
AsciiOutput( sFileName, '' );
AsciiOutput( sFileName, Expand('If( DimensionExists( ''%sDim%'' ) = 0 );') );
AsciiOutput( sFileName, Expand('    DimensionCreate( ''%sDim%'' );') );
AsciiOutput( sFileName, Expand('    If( ''%sDim%'' @<> ''%sHier%'' );') );
AsciiOutput( sFileName, Expand('        HierarchyCreate( ''%sDim%'', ''%sHier%'' );') );
AsciiOutput( sFileName, '    EndIf;' );
AsciiOutput( sFileName, Expand('ElseIf( ''%sDim%'' @<> ''%sHier%'' );') );
AsciiOutput( sFileName, Expand('    If( HierarchyExists( ''%sDim%'', ''%sHier%'' ) = 0 );') );
AsciiOutput( sFileName, Expand('        HierarchyCreate( ''%sDim%'', ''%sHier%'' );') );
AsciiOutput( sFileName, '    EndIf;' );
AsciiOutput( sFileName, 'EndIf;' );
...
If( pDimInfo = 1 );
    ...
    AsciiOutput( sFileName, Expand('DimensionSortOrder( ''%vDim%'', ''%sSORTCOMPONENTSTYPE%'', ''%sSORTCOMPONENTSSENSE%'', ''%sSORTELEMENTSTYPE%'', ''%sSORTELEMENTSSENSE%'' );') );
    AsciiOutput( sFileName, Expand('CellPutS( ''%sDefHier%'', ''}DimensionProperties'', ''%vDim%'', ''DEFAULT_HIERARCHY'' );') );
    AsciiOutput( sFileName, Expand('CellPutS( ''%sDefMember%'', ''}HierarchyProperties'', ''%vDim%'', ''hierarchy0'', ''defaultMember'' );') );
    AsciiOutput( sFileName, Expand('CellPutN( %sHierLevels%, ''}HierarchyProperties'', ''%vDim%'', ''hierarchy0'', ''LevelsToHierarchize'' );') );
    nCtr                = 0;
    While( nCtr         <= 20 );
        sLvl            = 'Level0' | If( nCtr < 10, '0', '' ) | NumberToString( nCtr ); 
        sLvlVal         = Expand('%'| Expand('s%sLvl%') |'%');
        If( sLvlVal     @<> '' );
            AsciiOutput( sFileName, Expand('CellPutS( ''%sLvlVal%'', ''}HierarchyProperties'', ''%vDim%'', ''hierarchy0'', ''%sLvl%'' );') );
        EndIf;
        nCtr            = nCtr + 1;
    End;
    If( CubeExists( '}DimensionAttributes' ) = 1 );
        ...
        While( nCtr     <= DimSiz( '}DimensionAttributes' ) );
            ...
            If( sAttrTyp @= 'N' );
                AsciiOutput( sFileName, Expand('DimensionAttrPutN( %sAttrVal%, ''%vDim%'', ''%sAttr%'' );') );
            Else;
                AsciiOutput( sFileName, Expand('DimensionAttrPutS( ''%sAttrVal%'', ''%vDim%'', ''%sAttr%'' );') );
            EndIf;
            nCtr        = nCtr + 1;
        End;
    EndIf;
...
If( pSub = 1 & DimensionExists( '}Subsets_' | sDim ) = 1 );
    If( sDim @= vDim );
        sMDX = Expand('{Except( {TM1SubsetAll( [}Subsets_%sDim%] )}, {TM1FilterByPattern( {TM1SubsetAll( [}Subsets_%sDim%] )}, "*:*" )} )}');
    Else;
        sMDX = Expand('{TM1FilterByPattern( {TM1SubsetAll( [}Subsets_%sDim%] )}, "%sHier%:*" )}');
    EndIf;
    If( SubsetExists( '}Subsets_' | sDim, cTempSub ) = 1 );
        SubsetMDXSet( '}Subsets_' | sDim, cTempSub, sMDX );
    Else;
        SubsetCreatebyMDX( cTempSub, sMDX, '}Subsets_' | sDim, 1 );
    EndIf;
    nMax = SubsetGetSize( '}Subsets_' | sDim, cTempSub );
    If( nMax >= 1 );
        AsciiOutput( sFileName2, '' );
        AsciiOutput( sFileName2, cHashLine );
        ...
        nCtr = 1;
        While( nCtr <= nMax );
            sSub        = SubsetGetElementName( '}Subsets_' | sDim, cTempSub, nCtr );
            If( Scan( ':', sSub ) > 0 );
            ...
            If( sMDX @<> '' );
                # create by MDX
                AsciiOutput( sFileName2, Expand('If( SubsetExists( ''%vDim%'', ''%sSub%'' ) = 0 );') );
                AsciiOutput( sFileName2, Expand('    SubsetCreatebyMDX( ''%sSub%'', ''%sMDX%'', ''%vDim%'', 0 );') );
                AsciiOutput( sFileName2, 'Else;' );
                AsciiOutput( sFileName2, Expand('    SubsetMDXSet( ''%vDim%'', ''%sSub%'', ''%sMDX%'' );') );
                AsciiOutput( sFileName2, 'EndIf;' );
            Else;
                # loop members
                AsciiOutput( sFileName2, Expand('If( SubsetExists( ''%vDim%'', ''%sSub%'' ) = 0 );') );
                AsciiOutput( sFileName2, Expand('    SubsetCreate( ''%vDim%'', ''%sSub%'' );') );
                AsciiOutput( sFileName2, 'Else;' );
                AsciiOutput( sFileName2, Expand('    SubsetDeleteAllElements( ''%vDim%'', ''%sSub%'' );') );
                AsciiOutput( sFileName2, 'EndIf;' );
